...
te_out<-OUT.3(sss988$TELEBET.BALANCE)

TS<-function(sbalance){ 
  out<-c(1)
  sbalance<-sbalance[-out]
  return(sbalance) }
ma_ts<-TS(sss988$MAIN.BALANCE)[1:103]
ma_ts_orginal<-TS(sss988$MAIN.BALANCE)
ma_test<-TS(sss988$MAIN.BALANCE)[104:110]
...
accuracy(predict(fit_ets,7),ma_test)#810156.3
